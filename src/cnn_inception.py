...
class Net(nn.Module):
    def __init__(self):
        ...
        # Inception layers
        self.conv1_1 = nn.Conv2d(1, 32, (1, 1))
        self.conv1_3 = nn.Conv2d(1, 16, (3, 3), padding=1, padding_mode='circular')
        self.conv1_5 = nn.Conv2d(1, 16, (5, 5), padding=2, padding_mode='circular')
        self.conv2 = nn.Conv2d(64, 16, (3, 3), padding=1, padding_mode='circular')
        #self.conv2_1 = nn.Conv2d(64, 16, (1, 1))
        #self.conv2_3 = nn.Conv2d(64, 8, (3, 3), padding=1, padding_mode='circular')
        #self.conv2_5 = nn.Conv2d(64, 8, (5, 5), padding=2, padding_mode='circular')
        self.conv3 = nn.Conv2d(16, 4, (3, 3), padding=1, padding_mode='circular')
        self.conv4 = nn.Conv2d(4, 1, (3, 3), padding=1, padding_mode='circular')
        self.evolve_weights = torch.tensor([[1,1,1],[1,10,1],[1,1,1]]).view(1,1,3,3) 

    def forward(self, x):
        x = F.relu(torch.cat((self.conv1_1(x), self.conv1_3(x), self.conv1_5(x)), dim=1))
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        x = torch.sigmoid(self.conv4(x))
        x = F.conv2d(x, self.evolve_weights, padding=1)
